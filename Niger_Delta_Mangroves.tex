...
\begin{lstlisting}[language=bash,caption=GRASS GIS script - 2 for computing the NDVI,style=mystyle,label={lst01}]
# Calculation of VI
...
d.legend raster=lsat8_2013.ndvi range=-1,1 title=NDVI title_fontsize=12 font=Helvetica fontsize=10 -t -b border_color=gray thin=8 -s label_step=0.1 -d
d.out.file output=Nigeria_NDVI_2013 format=jpg --overwrite
\end{lstlisting}

\begin{lstlisting}[language=bash,caption=GRASS GIS script - 2 for computing the ARVI,style=mystyle,label={lst01}]
# Calculation of ARVI
# arvi( redchan, nirchan, bluechan )
# ARVI: Atmospheric Resistant Vegetation Index = (NIR - (2.0*Red - Blue)) / ( NIR + (2.0*Red - Blue))
...
d.mon wx2
#r.colors lsat8_2013.arvi color=haxby
r.colors lsat8_2013.arvi color=bcyr
# r.colors --help
...
\subsection{Global Environment Monitoring Index }

\begin{verbatim}
n = ( 2*( NIR^2 - Red^2) + 1.5*NIR + 0.5*Red ) / ( NIR + Red + 0.5 )
\end{verbatim}

\subsection{IPVI: Infrared Percentage Vegetation Index}
...
\makebox[0.90\linewidth][r]{%
	...
	\begin{subfigure}[b]{.55\textwidth}
		...
		\caption{2022}
	\end{subfigure}%
}\caption{NDVI computed from the Landsat 8-9 OLI/TIRS images of Niger Delta for six years: (\textbf{a}) 2013/12/20, (\textbf{b}) 2015/12/26, (\textbf{c}) 2018/02/21, (\textbf{d}) 2020/01/22, (\textbf{e}) 2021/12/18, (\textbf{f}) 2022/12/29.}\label{fig:03}
\end{figure}
